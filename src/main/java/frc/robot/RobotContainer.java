...
public class RobotContainer {
  ...
  private void configureBindings() {
        ...
        drivetrain.applyRequest(
            () ->
                drive
                    ...
                    .withRotationalRate(
                        -driver.getRightX() * maxAngularRate) // CCW with negative X (left)
            ));
    ...
    // Superstructure presets (arm + flywheel move together). These use whileTrue, so the command
    // runs only while the button is held and is canceled on release. The preset commands are
    // runRepeatedly with no default command or whenCanceled, so on release the TalonFX simply holds
    // its last setpoint - behaving like onTrue today, but leaving a clean hook to add a default
    // command or whenCanceled cleanup later without changing the binding.
   ...
   * scheduler enforces them there. No tag visible: {@code DriveToTag} holds zero velocity. Release
   * the button: the whole thing cancels.
   */
  private Command autoScore() {
    return Command.noRequirements(
            coroutine -> {
              coroutine.awaitAll(
                  new DriveToTag(drivetrain), flywheel.spinUpAndWait(), arm.scoring());
